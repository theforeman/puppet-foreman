...
# configuration is in /etc/foreman/puppet.yml
...
$settings_file = "/etc/foreman/puppet.yml"
...
Puppet::Reports.register_report(:foreman) do
  ...
  def process
    begin
      ...
      if http.use_ssl?
        if SETTINGS[:foreman_ssl_ca] && !SETTINGS[:foreman_ssl_ca].empty?
          http.ca_file = SETTINGS[:foreman_ssl_ca]
          http.verify_mode = OpenSSL::SSL::VERIFY_PEER
        else
          http.verify_mode = OpenSSL::SSL::VERIFY_NONE
        end
        if SETTINGS[:foreman_ssl_cert] && !SETTINGS[:foreman_ssl_cert].empty? && SETTINGS[:foreman_ssl_key] && !SETTINGS[:foreman_ssl_key].empty?
