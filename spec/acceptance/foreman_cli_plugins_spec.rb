...
describe 'Scenario: install foreman-cli + plugins without foreman' do
  ...
  context 'for standard plugins' do
    ...
    ['ansible', 'bootdisk', 'discovery', 'google', 'puppet', 'remote_execution', 'salt', 'ssh', 'tasks', 'templates', 'webhooks'].each do |plugin|
  ...
  if fact('os.family') == 'RedHat'
    context 'for katello' do
      ...
      ['katello', 'foreman_rh_cloud'].each do |plugin|
